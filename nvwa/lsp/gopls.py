...
GOPLS_PATH = os.environ.get("LSP_GOPLS_PATH", "/opt/tools/go/path/bin/gopls")
...
class GoLanguageServer(LanguageServer):
    ...
    def _start(self):
        ...
        self.proc = subprocess.Popen(
            [GOPLS_PATH] + [
                "-remote=auto",  # 自动使用远程模式提升性能
                "-logfile", f"{os.path.expanduser('~')}/.gopls.log",  # 日志文件路径
                "-v"  #  verbose模式便于调试
            ],
